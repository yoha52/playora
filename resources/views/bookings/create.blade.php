<x-layouts.app :title="__('general.add_booking')">
    <div class="px-4 pt-6">
        <!-- Header -->
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('general.add_booking') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('general.create_new_booking') }}</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <form action="{{ route('bookings.store') }}" method="POST" class="p-6">
                ...
                @include('bookings._form', ['booking' => null, 'courts' => $courts, 'selectedCourt' => $selectedCourt, 'selectedDate' => $selectedDate])
