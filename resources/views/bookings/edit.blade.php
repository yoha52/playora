<x-layouts.app :title="__('general.edit_booking')">
    <div class="px-4 pt-6">
        <!-- Header -->
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('general.edit_booking') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('general.update_booking_info') }}</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <form action="{{ route('bookings.update', $booking) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')

                @include('bookings._form', ['booking' => $booking, 'courts' => $courts, 'selectedCourt' => $booking->court, 'selectedDate' => $booking->date->format('Y-m-d')])

                <div class="mt-6 flex justify-end gap-3">
                    <a href="{{ route('bookings.index') }}" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded border border-gray-300 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                        {{ __('general.cancel') }}
                    </a>
