<x-layouts.app :title="__('Categories')">
    <div class="px-4 pt-6">
        ...
        <div class="mb-4 flex items-center justify-between">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('general.categories') }}</h1>
            <button id="create-category-btn" type="button" class="text-white bg-brand-500 hover:bg-brand-600 focus:ring-4 focus:ring-brand-300 font-medium rounded text-sm px-5 py-2.5 dark:bg-brand-600 dark:hover:bg-brand-700 focus:outline-none dark:focus:ring-brand-800">
                ...
                <div class="w-full md:w-1/2">
                    <form action="{{ route('categories.index') }}" method="GET" class="flex items-center">
                        ...
                        <div class="relative w-full">
                            ...
                            <input type="text" id="search" name="search" value="{{ request('search') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-brand-500 focus:border-brand-500 block w-full pl-10 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-brand-500 dark:focus:border-brand-500" placeholder="Search categories...">
                        </div>
                        <button type="submit" class="ml-2 text-white bg-brand-500 hover:bg-brand-600 focus:ring-4 focus:ring-brand-300 font-medium rounded text-sm px-4 py-2 dark:bg-brand-600 dark:hover:bg-brand-700 focus:outline-none dark:focus:ring-brand-800">
                            {{ __('general.search') }}
                        </button>
                        @if(request('search'))
                            <a href="{{ route('categories.index') }}" class="ml-2 text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-300 font-medium rounded text-sm px-4 py-2 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700 border border-gray-300">
                                {{ __('general.clear') }}
                            </a>
                        @endif
                    </form>
                ...
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-sm text-body bg-neutral-secondary-soft border-b rounded-base border-default">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">{{ __('general.created_at') }}</th>
                    ...
                    <tbody>
                        @forelse($categories as $category)
                            <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <td class="px-4 py-3">
                                    <img src="{{ $category->getFirstMediaUrl('picture') }}" alt="{{ $category->name }}" class="w-12 h-12 rounded-lg object-cover">
                                </td>
                                ...
                                <td class="px-4 py-3 flex items-center justify-end gap-2">
                                    <button onclick="editCategory({{ $category->id }})" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded text-sm px-3 py-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                        {{ __('general.edit') }}
                                    </button>
                                    <button onclick="confirmDelete({{ $category->id }}, '{{ addslashes($category->name) }}')" class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded text-sm px-3 py-2 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-900">
                                        {{ __('general.delete') }}
                                    </button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                    ...
                                    <p class="mt-1">Get started by creating a new category.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <!-- Create/Edit Modal -->
    <x-crud.modal id="category-modal" :title="__('general.add_category')">
        <form id="category-form" action="{{ route('categories.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded focus:ring-brand-500 focus:border-brand-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-brand-500 dark:focus:border-brand-500" required>
                    @error('name')
                ...
                <div class="flex items-center">
                    <input type="checkbox" id="active" name="active" value="1" checked class="w-4 h-4 text-brand-600 bg-gray-100 border-gray-300 rounded focus:ring-brand-500 dark:focus:ring-brand-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="active" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('general.active') }}</label>
                </div>

                <!-- Image Upload -->
                <div>
                    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('general.image') }}</label>
            ...
            <div class="flex justify-end gap-2">
                <button type="button" class="modal-cancel-btn text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded border border-gray-300 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                    {{ __('general.cancel') }}
                </button>
        ...
        window.addEventListener('load', function() {
            ...
            function initFilePond() {
                if (typeof window.FilepondManager !== 'undefined') {
                    pond = window.FilepondManager.init('#category-image');
                } else if (typeof window.FilePond !== 'undefined') {
                    const inputElement = document.querySelector('#category-image');
                    if (inputElement) {
                        pond = window.FilePond.create(inputElement, {
                            server: {
            ...
            ModalManager.setupCloseButtons('category-modal');
            ...
            document.getElementById('create-category-btn').addEventListener('click', function() {
                document.querySelector('#category-modal .modal-title').textContent = i18n.general.add_category;
            ...
            window.editCategory = function(id) {
                fetch(`/categories/${id}/edit`)
                    ...
                    .then(data => {
                        document.querySelector('#category-modal .modal-title').textContent = i18n.general.edit_category;
            ...
            window.confirmDelete = function(id, name) {
                ...
                document.querySelector('#delete-modal .delete-form').action = `/categories/${id}`;
