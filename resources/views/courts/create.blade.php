<x-layouts.app :title="__('general.add_court')">
    <div class="px-4 pt-6">
        <!-- Header -->
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('general.add_court') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Create a new court with schedule and pricing</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <form action="{{ route('courts.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf

                @include('courts._form', ['court' => null, 'grounds' => $grounds, 'categories' => $categories])
