<x-layouts.app :title="__('general.edit_ground')">
    <div class="px-4 pt-6">
        <!-- Header -->
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('general.edit_ground') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update ground information and facilities</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <form action="{{ route('grounds.update', $ground) }}" method="POST" enctype="multipart/form-data" class="p-6">
                @csrf
                @method('PUT')

                @include('grounds._form', ['ground' => $ground])
