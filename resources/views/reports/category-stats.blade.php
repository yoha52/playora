<x-layouts.app :title="__('general.category_stats_report')">
    <div class="px-4 pt-6">
        <!-- Header -->
        <div class="mb-4 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('general.category_stats_report') }}</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('general.category_stats_report_desc') }}</p>
            </div>
        </div>
        ...
        <div x-data="{ open: true }" class="bg-white dark:bg-gray-800 shadow-md rounded-lg mb-6 border-t-2 border-brand-500">
            ...
            <div x-show="open" x-collapse class="px-4 pb-4">
                <form action="{{ route('reports.category-stats') }}" method="GET" class="flex flex-wrap items-end gap-4">
                    <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request()->hasAny(['from_date', 'to_date']))
                            <a href="{{ route('reports.category-stats') }}" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-gray-300 font-medium rounded text-sm px-5 py-2.5 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 border border-gray-300">
                                {{ __('general.clear') }}
                            </a>
                        @endif
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @forelse($stats as $category)
                                <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-3">
                                            <img src="{{ $category->getFirstMediaUrl('picture') }}" alt="{{ $category->name }}" class="w-10 h-10 rounded object-cover">
                                            <span class="font-medium text-gray-900 dark:text-white">{{ $category->name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 text-center">{{ $category->courts_count }}</td>
                                    <td class="px-4 py-3 text-center font-medium text-gray-900 dark:text-white">{{ $category->total_bookings }}</td>
                                    <td class="px-4 py-3 text-center">
                                        <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{{ $category->confirmed_bookings }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-center">
                                        <span class="bg-green-100 text-green-800 text-xs font-medium px-2 py-0.5 rounded dark:bg-green-900 dark:text-green-300">{{ $category->completed_bookings }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-center">
                                        <span class="bg-red-100 text-red-800 text-xs font-medium px-2 py-0.5 rounded dark:bg-red-900 dark:text-red-300">{{ $category->cancelled_bookings }}</span>
                                    </td>
                                    <td class="px-4 py-3 text-right font-medium text-gray-900 dark:text-white">{{ formatNumber($category->total_revenue ?? 0) }}</td>
                                    <td class="px-4 py-3 text-right text-green-600 dark:text-green-400">{{ formatNumber($category->collected_amount ?? 0) }}</td>
                                    <td class="px-4 py-3 text-right text-red-600 dark:text-red-400">{{ formatNumber(($category->total_revenue ?? 0) - ($category->collected_amount ?? 0)) }}</td>
                                </tr>
                            @empty
                                <tr>
